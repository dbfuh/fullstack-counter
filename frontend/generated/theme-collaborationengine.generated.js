...
import stylesCss from 'themes/collaborationengine/styles.css';
...
window.Vaadin['_vaadintheme_collaborationengine_globalCss'] = window.Vaadin['_vaadintheme_collaborationengine_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_collaborationengine_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_collaborationengine_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_collaborationengine_globalCss'].push(target);
  }
  if (!document['_vaadintheme_collaborationengine_componentCss']) {
    
    document['_vaadintheme_collaborationengine_componentCss'] = true;
  }
